...
- **Motion**: Use animations where they genuinely add to the design — transitions, load reveals, and interactive feedback almost always do. Prioritize CSS-only for HTML prototypes. Use the Motion library (`skills/lb-motion-skill/`) for React. High-impact moments: staggered page load reveals, hover states that surprise, route/modal entrance+exit. Skip animations that feel forced or distracting. Polished and alive, not busy.
