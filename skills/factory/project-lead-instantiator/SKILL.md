...
description: Create/reuse a standalone per-project Project Lead session using a deterministic project-named session key (agent:project-lead:<projectId>), optionally update the project’s project-state.json, and print the exact openclaw commands (tui). Use when Kelly needs to start/route a project to Project Lead without spawning subagents.
---
...
- Prints:
  ...
  - the `openclaw tui --session ...` follow-up command (check-in)
