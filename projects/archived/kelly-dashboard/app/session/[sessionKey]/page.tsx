...
async function getSessionData(sessionKey: string): Promise<SessionData | null> {
  ...
  try {
    ...
    // For subagents, scan all projects (including active/ and archived/ subdirs)
    // to find which one contains this session
    async function findProjectStatePaths(rootDir: string): Promise<string[]> {
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          // Check if this dir has a project-state.json
          const statePath = path.join(dirPath, "project-state.json")
          try {
            await fs.access(statePath)
            results.push(statePath)
          } catch {
            // Not here, check subdirs (e.g., active/notelite, archived/foo)
            try {
              const subEntries = await fs.readdir(dirPath, { withFileTypes: true })
              for (const subEntry of subEntries) {
                if (!subEntry.isDirectory()) continue
                const subStatePath = path.join(dirPath, subEntry.name, "project-state.json")
                try {
                  await fs.access(subStatePath)
                  results.push(subStatePath)
                } catch {
                  // Not here either
                }
              }
