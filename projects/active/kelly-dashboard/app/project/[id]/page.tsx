...
const PROJECT_FOLDERS = ['active', 'ideas', 'shipped', 'archived']

async function getProjectState(projectId: string): Promise<ProjectState | null> {
  // Try to find project-state.json in all project folders
  for (const folder of PROJECT_FOLDERS) {
    try {
      const projectStatePath = path.join(PROJECTS_ROOT, folder, projectId, 'project-state.json')
      const contents = await fs.readFile(projectStatePath, 'utf8')
      return JSON.parse(contents)
    } catch (error) {
      // If ENOENT, try next folder
  ...
  console.error(`Project state not found for ${projectId} in any folder`)
  return null
}
...
async function getStoriesData(projectId: string, storiesPath?: string): Promise<any> {
  ...
  for (const folder of PROJECT_FOLDERS) {
    try {
      const testPath = path.join(PROJECTS_ROOT, folder, projectId, 'project-state.json')
  ...
  if (!projectFolder) {
    console.error(`Could not find project folder for ${projectId}`)
    return null
  }

  const projectRoot = path.join(PROJECTS_ROOT, projectFolder, projectId)
